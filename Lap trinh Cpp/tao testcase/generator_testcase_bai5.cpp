...
{
    vector<string> ho = {"nguyen", "tran", "Le", "Pham", "huynh", "Phan", "Vo", "Nguyen"};
    vector<string> tenDem = {"Van", "thi", "minh", "Thu", "Hoang", "Duc"};
    vector<string> ten = {"anh", "Binh", "chien", "dung", "Giang", "hieu", "khanh", "Lan", "Viet"};

    string name = ho[rand() % ho.size()] + " ";
    int numTenDem = 1 + (rand() % 2);
    for (int i = 0; i < numTenDem; ++i)
    {
        int numSpace = rand() % 3 + 1;
        name += string(numSpace, ' ');  
...
void generateTestFile(int fileIndex, int numRecords)
{
    ...
    if (fileIndex < 10)
    {
        inputFileName.insert(5, 1, '0');
        outputFileName.insert(6, 1, '0');
    }
